...
class ThesisCommitteeInfoCollection:
    def __init__(self) -> None:
        self.thesis_assignments: List[ThesisCommitteeInfo] = []

    def proccess_info(self, thesis_assignments: List[dict]):
        '''Main function to analize all teaching assignments'''
        for thesis_assignment in thesis_assignments:
            self.analize_row(thesis_assignment)

    def analize_row(self, thesis_committee: dict):
        '''Compute and save all the important data of a thesis assignment'''
        # get data
        print(thesis_committee)
        print('Aqui')

        date = thesis_committee['date']
        time = thesis_committee['time']
        place = thesis_committee['place']['name']
        student = thesis_committee['thesis']['student']

        # subject_name = thesis_assignment['subject_description']['name']
...
class TC_CSV_GENERATOR():
    ...
    def generate_csv(self):
        ...
        tc_info = ThesisCommitteeInfoCollection()
        tc_info.proccess_info(data)

        # rows = self.construct_fields(tc_info)
